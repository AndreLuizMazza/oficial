...
function SaibaMaisLink({ onDesktop, onMobile }) {
  ...
  return (
    <Tooltip
      label="Faixas progressivas após 500 contratos. Clique para ver a explicação."
      disableOnTouch
    >
      <button
        ref={btnRef}
...
const EXTRAS_COMUNS = [
  {
    ...
    description:
      "Envios em escala para cobranças, lembretes e relacionamento com clientes, via provedores oficiais.",
  },
];
...
const PLANS_RAW = [
  {
    ...
    features: [
      {
        icon: Receipt,
        label: "Gestão de Óbitos",
        status: { type: "sim", text: "Sim" },
      },
      {
        icon: Workflow,
      ...
      {
        icon: Cable,
        label: "Integração Meios de Pagamento",
        status: { type: "sim", text: "Sim" },
      },
      {
        icon: Globe2,
        label: "Memorial Digital",
        status: { type: "sim", text: "Sim" },
      }, // incluso
      {
        icon: Store,
        label: "Gestão de Planos",
        status: { type: "nao", text: "Não" },
      },
      {
        icon: Smartphone,
        label: "APP Cobrador & Vendedor",
        status: { type: "nao", text: "Não" },
      },
      {
        icon: Users,
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        icon: Users,
        label: "APP Associado",
        status: { type: "adicional", text: "Add-on" },
      },
      {
        icon: Globe,
        label: "Site",
  ...
  return (
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {items.map(({ icon: Icon, label, sub }, i) => (
        <div
          key={i}
          className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface)] p-3 flex items-center gap-3"
        >
          <span className="inline-flex w-9 h-9 items-center justify-center rounded-xl border border-[var(--c-border)] bg-[var(--c-surface-2)]">
            <Icon
              className="w-5 h-5 text-[color:var(--c-muted)]"
              aria-hidden="true"
            />
          </span>
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 relative pb-24">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-[var(--c-border)] bg-[var(--c-surface)]">
                <Info
                  className="w-4.5 h-4.5 text-[color:var(--c-muted)]"
                  aria-hidden="true"
                />
        ...
        {plans.map((p) => {
          ...
          return (
            <CardMotion
              key={p.id}
              className={clsx(
                "relative card p-6 flex flex-col h-full overflow-hidden border border-[var(--c-border)]",
            ...
            >
              ...
              {/* Conteúdo do card (flex-1 para alinhar CTA no rodapé) */}
              <div className="flex-1 flex flex-col">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <span className="inline-flex items-center justify-center w-12 h-12 rounded-xl border border-[var(--c-border)] bg-[var(--c-surface-2)] shadow-sm">
                      <p.icon
                        className="w-6 h-6 text-[color:var(--c-muted)]"
                        aria-hidden="true"
                      />
                    </span>
                    <div>
                      ...
                      <div className="text-[11px] text-[color:var(--c-muted)] mt-0.5">
                        <div>
                          <span className="line-through opacity-70">
                            {fmtBRL(anualSemDesconto)}
                          </span>{" "}
                          → <strong>{fmtBRL(anualComDesconto)}</strong> no ano
                        </div>
                        <div>
                          equivale a{" "}
                          <strong>{fmtBRL(mensalEquivalenteNoAnual)}</strong> /
                          mês
                        </div>
                      </div>
                    )}
                ...
                <div className="mt-3 text-xs text-[color:var(--c-muted)]">
                  {showStartNote && (
                    <span>
                      Preço <strong>fixo</strong> até{" "}
                      <strong>500 contratos ativos</strong>.
                    </span>
                  )}
                  {showTierNoteProEnt && (
                    <span>
                      Mantém este valor{" "}
                      <strong>até 500 contratos ativos</strong>.{" "}
                      <SaibaMaisLink
                        onDesktop={() => setOpenModal(true)}
                        onMobile={(rect) =>
                          setPopover({ open: true, anchorRect: rect })
                        }
                      />
                    </span>
                  )}
              ...
              {/* CTA alinhado ao rodapé do card */}
              <div className="mt-6 pt-4 border-t border-[var(--c-border)]">
                <Link
                  to={p.cta.to}
                  className={clsx(
                    "btn w-full transition text-white border-transparent",
                    // laranja fixo para todos os planos
                    "bg-[#f97316] hover:bg-[#ea580c] focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#ea580c]"
                  )}
      ...
      <section id="extras" className="mt-10 card p-5">
        ...
        <p className="muted text-sm mt-1">
          Destaque para a <strong>integração com WhatsApp ilimitado</strong>,
          ideal para lembretes de pagamento, avisos e relacionamento em escala.
        </p>
            ...
            >
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-[var(--c-border)] bg-[var(--c-surface)]">
                  <MessageCircle
                    className="w-4 h-4 text-[var(--c-primary)]"
                    aria-hidden="true"
                  />
                </span>
                <div>
        ...
        <p className="text-xs text-[color:var(--c-muted)] mt-2">
          Contratados à parte e ativados sob demanda. O{" "}
          <strong>App do Associado</strong> está disponível como{" "}
        ...
        <div className="card p-5 space-y-6">
          ...
          <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface)] p-4">
            ...
            <dl className="grid sm:grid-cols-3 gap-3 text-sm">
              <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface-2)] p-3">
                ...
                <dd className="mt-1">
                  <strong>Mensal</strong> ou <strong>Anual</strong> (15% OFF).
                </dd>
              </div>
              <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface-2)] p-3">
                <dt className="muted">Usuários incluídos</dt>
                <dd className="mt-1">
                  Start/Pro: <strong>5</strong> • Enterprise: <strong>10</strong>
                  <br />
                  Usuário adicional:{" "}
                  <strong>{fmtBRL(EXTRA_USER_PRICE)}/mês</strong>.
                </dd>
              </div>
              <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface-2)] p-3">
                <dt className="muted">Setup inicial</dt>
                <dd className="mt-1">
                  A partir de <strong>{fmtBRL(600)}</strong> —{" "}
                  <em>pode variar</em> conforme integrações,
                  <em> número de usuários</em> e{" "}
                  <em>escopo de migração de dados</em>.
                </dd>
              </div>
            </dl>
          </div>
          ...
          <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface)] p-4">
            ...
            <div className="grid md:grid-cols-2 gap-3 text-sm">
              <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface-2)] p-3">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center px-2 py-0.5 text-[11px] rounded-md bg-emerald-100/60 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300">
                    Sem custo
                  </span>
                  <span className="font-medium">
                    Sem registro bancário / Baixa manual
                  </span>
                </div>
                <p className="muted mt-1">
                  Gere cobranças <strong>sem registro bancário</strong> para{" "}
                  <strong>recebimento manual</strong> sem custo. A{" "}
                  <strong>baixa manual</strong> de <strong>PIX</strong> ou{" "}
                  <strong>boletos</strong> também não tem custo.
                </p>
              </div>

              <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface-2)] p-3">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center px-2 py-0.5 text-[11px] rounded-md bg-amber-100/70 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">
                    Tarifa do banco
                  </span>
                  <span className="font-medium">Pagamento via banco</span>
                </div>
                <p className="muted mt-1">
                  Há custos apenas quando o cliente paga pelo banco (tarifas do
                  meio de pagamento). Consulte{" "}
                  <Link to="/taxas" className="underline">
                    Taxas &amp; Cobrança
                  </Link>
                  .
                </p>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[
            ...
            },
            {
              ...
              a: (
                <>
                  Não. É um add-on de custo fixo mensal (por faixa), via
            ...
            },
            {
              ...
              a: (
                <>
                  Sim. Ajudamos na importação de clientes, contratos,
                  carnês/boletos e histórico básico.
                </>
              ),
            },
            {
              q: "Posso mudar de plano depois?",
              a: "Pode. O ajuste acompanha sua faixa de contratos ativos.",
            },
            {
              q: "A baixa manual tem algum custo?",
              a: "Não. Só há cobrança quando o pagamento acontece pelo banco.",
            },
            {
              q: "Quais são as taxas quando o cliente paga pelo banco?",
              a: (
                <>
                  As tarifas dependem do meio de pagamento. Consulte a página{" "}
                  <Link to="/taxas" className="underline">
                    Taxas &amp; Cobrança
                  </Link>
                  .
            ...
            },
            {
              ...
              a: (
                <>
                  Sim. Você pode contratar qualquer um separadamente conforme
                  necessidade.
                </>
              ),
            },
            {
              ...
              a: (
                <>
                  Sim. Disponibilizamos API para integrações (veja a{" "}
                  <a
            ...
            {
              q: "Existe fidelidade?",
              a: "Trabalhamos com fidelidade de 1 ano.",
            },
            {
              q: "Como funciona o suporte?",
              a: "Atendemos com SLA de até 4h úteis, independentemente do plano contratado.",
            },
          ].map((item, i) => (
      ...
      >
        <p>
          A partir de <strong>501 contratos ativos</strong>, aplicamos uma
          tabela de faixas progressivas com valor adicional por bloco de
          contratos. Isso mantém o custo proporcional ao seu crescimento, sem
          surpresas. Para receber a tabela detalhada e uma simulação sob
      ...
      >
        <h3 className="text-base font-semibold">
          Como funcionam as faixas acima de 500 contratos
        </h3>
